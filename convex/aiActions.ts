...
import { getUserAnthropicApiKey } from "../lib/user-api-key";
...
async function generateMCPCode(params: {
  ...
  const result = await generateMCPFromOpenAPI(spec, params.userApiKey);
  return {
    code: result.code,
    tools: result.tools || [],
...
async function analyzeAPIDocumentation(
  ...
  try {
    const result = await generateMCPFromDocs(html, url, userApiKey);
    return {
...
async function analyzeGitHubRepoWithKey(
  owner: string,
  repo: string,
  userApiKey?: string
): Promise<{
  ...
  // Use Claude to analyze the code (with user's key if available)
  const result = await analyzeRepo(relevantFiles, userApiKey);
...
async function generateDocsHelper(
  ...
  return await generateDocs(code, tools, userApiKey);
}
...
export const generateFromOpenAPI = action({
  ...
  handler: async (ctx, { specUrl, userId }): Promise<any> => {
    ...
    const userApiKey = await getUserAnthropicApiKey(userId);
    ...
    // 4. Generate MCP tools using Claude API (with user's key if available)
...
export const generateFromDocsUrl = action({
  ...
  handler: async (ctx, { docsUrl, userId }): Promise<any> => {
    ...
    // 2. Use Claude to analyze documentation and generate MCP server (with user's key if available)
    const analysis = await analyzeAPIDocumentation(docsHtml, docsUrl, userApiKey || undefined);
...
export const generateFromGitHubRepo = action({
  ...
  handler: async (ctx, { repoUrl, userId }): Promise<any> => {
    ...
    // 3. Analyze repository structure (with user's key if available)
    const analysis = await analyzeGitHubRepoWithKey(owner, repoName, userApiKey || undefined);

    // 4. Generate MCP code (with user's key if available)
    const { code: generatedCode, tools, requiredApiKeys } = await generateMCPCode({
      name: repoData.name,
...
export const generateDocumentation = action({
  ...
  handler: async (ctx, { serverId }) => {
    ...
    // Get user's Anthropic API key if available
    const userApiKey = await getUserAnthropicApiKey(server.userId);

    // Generate documentation using Claude (with user's key if available)
    const docs = await generateDocsHelper(server.code, server.tools, userApiKey || undefined);
